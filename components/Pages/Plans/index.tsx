...
const PlanCard: React.FC<PlanCardProps> = (props) => {
  const { title, subTitle, types , isPopular , type} = props.planData;
 ...
 if(type === "curve"){
  return (
    <section className=" w-full mt-10  flex flex-col h-auto items-center transition-all relative">
       <div className="absolute -top-10 z-10 mx-auto w-min p-5 bg-white rounded-full border border-white-shade-4">
            <Diamonds />
          </div>
      <div className="  planCardTop w-full bg-top bg-cover flex flex-col gap-y-3  text-black p-5 pb-14   bg-white-shade-4 border border-white-shade-7 rounded-3xl " style={isPopular ? { backgroundImage: 'url("/images/plans/bg.png")' } : {}}>
